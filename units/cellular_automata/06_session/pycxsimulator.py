...
class GUI:
    ...
    def initGUI(self):
        ...
        self.p = DoubleVar(value=0.02)
        self.parameterSetters.append(self.p)
        ...
        for variableSetter in self.parameterSetters:
            ...
            lab = Label(can, width=25,height=1,text="p"" ",anchor=W,takefocus=0)

            lab.pack(side='left')
            
            ent = Entry(can, width=11)
            ent.insert(0, str(variableSetter.get()))
  
            ent.pack(side='left')
                
            can.pack(side='top')
            
            self.varEntries["p"]=ent
    ...
    def saveParametersCmd(self):
        for variableSetter in self.parameterSetters:
            variableSetter.set(float(self.varEntries["p"].get()))
